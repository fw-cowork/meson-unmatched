...
当前板端应用包括 D2 RGB LED、FU740 板级测试、M-mode CSR 检查和不返回 U-Boot 的
standalone runtime 检查；后两者通过 SiFive UART 驱动直接输出结果。公共层为后续
风扇、GPIO、时钟和其他外设测试提供复用边界。

本设计覆盖：

- U-Boot `go` 的调用约定、返回路径和 standalone 接管路径；
...
1. 使用 U-Boot 已完成的 DDR、时钟和引脚复用，快速加载并运行小型测试程序；应用可以
   沿用 U-Boot 栈并返回，也可以切换到私有栈并接管当前 hart。
...
- 可返回 runtime 限制完整镜像不超过 64 KiB；standalone runtime 在同一区域顶部保留
  16 KiB 栈，代码、数据和 BSS 不得超过前 48 KiB。
...
- `return` runtime 中，`baremetal_main()` 返回后 U-Boot 的栈、返回地址、必要寄存器和
  特权级必须仍然有效；`standalone` runtime 不允许恢复 U-Boot 命令循环。
...
U-Boot go
    ...
    v
arch/riscv/cpu runtime selected by app manifest
    |
    +--> return: preserve U-Boot ra/sp, clear .bss
    +--> standalone: private sp, clear .bss
    v
apps/<name>/main.c
    |
    ...
    |
    +--> return value in a0 --> U-Boot command loop
    +--> standalone loop    --> reset required
```
...
| 目录 | 职责 | 输出或接口 |
|---|---|---|
| `arch/riscv/cpu/` | U-Boot `go` 可返回/standalone 入口、链接布局 | `baremetal_runtimes` |
...
头文件、编译参数和链接输入。应用 manifest 保存输出名、应用私有源码、组件依赖，以及
可选的 runtime；省略 runtime 时默认为 `return`。
...
manifest 设置 `'runtime': 'standalone'` 时改用
[`start-standalone.S`](../../baremetal/arch/riscv/cpu/start-standalone.S)：

1. 把 `sp` 设置为链接脚本的 `__stack_top`；
2. 清零 `__bss_start` 到 `__bss_end`；
3. 保持 `a0/a1` 不变并调用 `baremetal_main()`；
4. C 入口如果意外返回，则进入永久循环，不执行 `ret`。

该入口不保存 U-Boot 的 `sp` 或 `ra`，因为恢复它们与 runtime 的接管语义冲突。当前
实现使用普通跳转循环，不使用 `wfi`，从而不依赖 S-mode/M-mode trap 和中断配置。
它只接管执行 `go` 的当前 hart，不负责启动其他 hart。
...
这些返回码只适用于 `return` runtime。返回只恢复 CPU 调用现场，不撤销对 PWM、GPIO、
时钟或其他 MMIO 的修改。修改关键 CSR、破坏 U-Boot 栈、改变特权级或进入永久循环，
都可能使返回失败。standalone 应用没有 `rc`，必须通过串口、MMIO 或调试器报告结果。
...
standalone 脚本
[`u-boot-go-standalone.lds`](../../baremetal/arch/riscv/cpu/u-boot-go-standalone.lds)
额外定义：

```text
0x84000000  .text/.rodata/.data/.bss
      ...   __image_end <= __stack_bottom
0x8400c000  __stack_bottom
            16 KiB private stack, grows downward
0x84010000  __stack_top
```
...
- `.comment`、`.note`、`.eh_frame` 和 `.riscv.attributes` 被丢弃，避免裸机镜像携带
  无法执行或无用的运行时元数据。
- `ASSERT()` 检查 `_start` 地址；可返回脚本检查 64 KiB 大小上限，standalone 脚本检查
  16 字节栈顶对齐以及 `__image_end <= __stack_bottom`。
...
standalone 检查还要求 ELF 导出 `__stack_bottom` 和 `__stack_top`，并把实际栈区间写入
`.check` 产物。
...
架构目录分别导出可返回和 standalone 启动依赖，再登记到 `baremetal_runtimes`。每个
runtime 包含：

- 对应的启动汇编静态库；
- `link_whole`，确保 `_start` 不会因归档库按符号裁剪而丢失；
- 对应的 `-T u-boot-go*.lds`、`--gc-sections`、`--no-relax` 等链接参数；
- 通用目标编译依赖。

应用循环读取 manifest 的 `runtime`，默认选择 `return`。因此应用 target 不需要列出
`start.S`、`.lds` 或 freestanding 链接选项，也不会把两套入口同时链接进一个镜像。
...
`unmatched-tests-artifacts`、`unmatched-standalone-artifacts`。新机器通过共享的
`populate_sdk` 安装器获得该工具链；没有安装器时才从固定的 Freedom-U-SDK 源码生成。
...
2. 创建该目录的 `meson.build`，声明 `name`、应用源码和组件依赖；需要私有栈且不返回时
   增加 `'runtime': 'standalone'`。
...
复制 `start.S`、链接脚本或硬件寄存器实现。当前只有 `riscv`、`fu740` 和 U-Boot `go`
一种加载组合，因此没有引入未实现的 board option 或多架构配置。
...
- standalone runtime 不保存 U-Boot 现场，不会返回命令行；只能通过开发板复位恢复。
- standalone 应用的私有栈只覆盖 `0x8400c000..0x84010000`，不得把它当作通用堆空间。
...
3. 对应 runtime 的 64 KiB 或 48 KiB + 16 KiB 栈边界 `ASSERT()` 通过；
4. 不依赖 libc、OpenSBI 或 U-Boot 私有符号；
5. 若包含纯算法，`meson test --suite baremetal` 通过；
6. `return` 应用按约定返回 `rc`；standalone 应用文档明确最后输出和复位恢复方式。
